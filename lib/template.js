...
const NAVIID_REG = new RegExp('^\\[\\[.\\]\\(#<span\\s+id\\s+=\\s+"v_([a-zA-Z]+)_([0-9]+)"></span>\\)\\]$');
...
function syncRepoRawLogs(helper, newVer, oldVer, rawLogs, changelog) {
    ...
    const prevTopId = '<span id = "PrevTopHash=' + topHash + '"></span>\n';
    ...
    for(var key in logs.data) {
        ...
        if(array.length > minLineCnt) {
            ...
            logs.navi.some(function(urlItem, index) {
                const ma = urlItem.match(NAVIID_REG);
                if(ma && ma[1] == key) {
                    logs.navi.splice(index, 1);
                    return true; // break
                }
            });
        }
