...
const RegExpPTH = new RegExp('^<span\\s+id\\s+=\\s+"PrevTopHash=([A-Fa-f0-9]*)"></span>$');
...
function updateChangelog(helper) {
    ...
    if(rawData.incType.isMajor) {
        incType = 'major';
    } else if(rawData.incType.isMinor) {
        incType = 'minor';
    } else if(rawData.incType.isPatch) {
        incType = 'patch';
    }
    ...
    return template.syncRepoRawLogs(helper, nextTag, fromTagSha, rawData, changelog);
}
