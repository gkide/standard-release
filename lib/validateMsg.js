...
function validateMsg(helper, commitMsg) {
    ...
    if(autoFix && helper.hasUpper(hType)) {
        const old = hType;
    ...
    if(checkObj.isDefault) {
        ...
        if(autoFix && helper.hasUpper(hScope)) {
    ...
    if(autoFix && checkResult.autofix) {
        hasWarnings = true;
    ...
    if(checkObj.isDefault) {
        ...
        if(autoFix && helper.isCharUpper(hSubject.charAt(0))) {
    ...
    if(cfgParser.failOnWarnings(msgRules) && hasWarnings) {
        helper.infoMsg("Abort for fail on warnings");
        return false;
    }
