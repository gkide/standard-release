...
exports.validateMsg = function(helper, msgData, msgFile) {
    ...
    const msgHBF = (msgData || '').split('\n').filter(function(str) {
        return str.indexOf('#') !== 0;
    }).join('\n');

    const msgHeader = msgHBF.split('\n').shift();
    ...
    if(MERGE_COMMIT.test(msgHeader)) {
        helper.infoMsg('Merge commit detected.');
        return true;
    }
    ...
    msgRules.type.forEach(function(item, idex) {
        try {
            ...
            skipHt = '';
            helper.warnMsg("Config rules for commit header type abnormal.");
        }
    });
  ...
  if (argInHelp) {
    console.log(config.helpMessage, msgHBF);
  } else if (msgHeader) {
