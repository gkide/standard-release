...
exports.validateMsg = function(helper, msgData, msgFile) {
    ...
    if(true) {
        console.log('typeAny: ' + msgRules.typeAny);
        console.log('typeIgnore: ' + msgRules.typeIgnore);
        console.log('typeMixedCase: ' + msgRules.typeMixedCase);
        console.log('headerMaxLength: ' + msgRules.headerMaxLength);
        msgRules.type.forEach(function (item, idex) {
            try {
                console.log('[' + item.name + ']: ' + item.skip + ', ' + item.isFilter);
            } catch(err) {
                helper.warnMsg("Config file error: 'commitRules'");
            }
        })
  ...
  var AUTO_FIX = config.autoFix && msgFile;
  ...
  var messageWithBody = (msgData || '').split('\n').filter(function(str) {
