'use strict';

module.exports = require('yargs')
    .wrap(90)
    .locale('en')
    .env('STANDARD_RELEASE')
    .pkgConf('standard-release')
    .usage('Usage: $0 [options]')
    .option('init', {
        alias: 'i',
        describe: 'Init standard-release for a git project',
        type: 'string',
        default: '$PWD'
    })
    .option('release', {
        alias: 'r',
        describe: 'Updating changelog, bump semantic version',
        type: 'boolean',
        default: false
    })
    .option('message', {
        alias: 'm',
        describe: 'Validate the given message or file',
        type: 'string',
        requiresArg: true
    })
    .option('changelog', {
        alias: 'c',
        describe: 'Read CHANGELOG from the given file',
        type: 'string',
        default: 'CHANGELOG.md'
    })
    .group([ 'init', 'message', 'release', 'changelog' ], 'Core:')
    .option('major', {
        alias: 'X',
        describe: 'Semantic version major',
        type: 'number',
        requiresArg: true
    })
    .option('minor', {
        alias: 'Y',
        describe: 'Semantic version minor',
        type: 'number',
        requiresArg: true
    })
    .option('patch', {
        alias: 'Z',
        describe: 'Semantic version patch',
        type: 'number',
        requiresArg: true
    })
    .option('pre-release', {
        alias: 'P',
        describe: "semantic version pre-release",
        type: 'string',
        default: ''
    })
    .option('build-number', {
        alias: 'B',
        describe: "semantic version build number",
        type: 'string',
        default: ''
    })
    .group([ 'major', 'minor', 'patch', 'pre-release', 'build-number' ], 'Semantic Version:')
    .hide('dev')
    .option('dev', {
        describe: 'For standard-release developer',
        type: 'boolean',
        default: false
    })
    .option('silent', {
        alias: 'x',
        describe: 'Do not print logs',
        type: 'boolean',
        default: false
    })
    .help()
    .alias('help', 'h')
    .version()
    .alias('version', 'v')
    .example('$0 -r', 'automatic update changelog & semantic version')
    .example('$0 -m "chore(release): subject-msg"', 'Is it a validate message');
