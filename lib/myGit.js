...
function getRawCommits(usrArgs) {
    const obj = shell.exec('git log --abbrev-commit -1');
    if(obj.code != 0) {
        return []; // no logs
    }
...
function getRepoDirectory(prjDir) {
    ...
    const obj = shell.exec('git rev-parse --show-toplevel');
    if(obj.code != 0) {
        return '';
    }
...
function repoHeadHash() {
    // abbrev top commit HASH
    const obj = shell.exec('git log --format=%h -1');
    if(obj.code != 0) {
        return '';
    }
    let hash = obj.stdout;
    hash = hash.replace(/^(\s*)([A-Fa-f0-9]*)(\n*)$/g, '$2');
    return hash;
}
...
exports.repoHeadHash = repoHeadHash;
